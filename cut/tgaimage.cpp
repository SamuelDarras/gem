...
cut::TGAImage::TGAImage(const int w, const int h, const int bpp) : w(w), h(h), bpp(bpp), data(w*h*bpp, 0) {}

bool cut::TGAImage::read_tga_file(const std::string filename) {
    std::ifstream in;
...
bool cut::TGAImage::load_rle_data(std::ifstream &in) {
...
bool cut::TGAImage::write_tga_file(const std::string filename, const bool vflip, const bool rle) const {
...
bool cut::TGAImage::unload_rle_data(std::ofstream &out) const {
...
cut::TGAColor cut::TGAImage::get(const int x, const int y) const {
    if (!data.size() || x<0 || y<0 || x>=w || y>=h)
        return {};
    return TGAColor(data.data()+(x+y*w)*bpp, bpp);
}

void cut::TGAImage::set(int x, int y, const TGAColor &c) {
    if (!data.size() || x<0 || y<0 || x>=w || y>=h) return;
    memcpy(data.data()+(x+y*w)*bpp, c.bgra, bpp);
}

void cut::TGAImage::flip_horizontally() {
    int half = w>>1;
...
void cut::TGAImage::flip_vertically() {
    int half = h>>1;
    for (int i=0; i<w; i++)
...
int cut::TGAImage::width() const {
    return w;
}

int cut::TGAImage::height() const {
    return h;
}
